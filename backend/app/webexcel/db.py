...
Миграции alembic сюда не заведены намеренно. Модуль поднимается через
`create_all`, как `app/bbc/db.py`, — и это не срезанный угол, а способ выкатить
раздел без ревизии, которая на проде выполняется до старта приложения и в
случае ошибки не даёт подняться вообще ничему.
"""
